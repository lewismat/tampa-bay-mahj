...
// Holly can paste HTML (e.g. generated by Claude) onto her inquiry page.
// Only signed-in admins can set it, but strip anything executable anyway —
// this HTML renders on a public page, so a stolen session shouldn't become stored XSS.
function sanitizeHTML(raw) {
  let h = String(raw || '').slice(0, 20000);
  h = h.replace(/<\s*(script|iframe|object|embed|link|meta|base)\b[\s\S]*?<\s*\/\s*\1\s*>/gi, '');
  h = h.replace(/<\s*(script|iframe|object|embed|link|meta|base)\b[^>]*\/?>/gi, '');
  h = h.replace(/\son\w+\s*=\s*"[^"]*"/gi, '').replace(/\son\w+\s*=\s*'[^']*'/gi, '')
       .replace(/\son\w+\s*=\s*[^\s>]+/gi, '');
  h = h.replace(/(href|src|action|formaction)\s*=\s*(["']?)\s*javascript:[^"'>\s]*\2/gi, '$1="#"');
  return h;
}
...
router.put('/api/inquiry-config', requireAuth, async (req, res) => {
  try {
    ...
    const cfg = {
      ...
      custom_html: sanitizeHTML(b.custom_html),
      custom_position: b.custom_position === 'below' ? 'below' : 'above',
    };
